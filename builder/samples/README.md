...
|--------|------------|------------|----------|-----------|
| changenet | `changenet/Dockerfile` | `nvcr.io/nvidia/deepstream:9.0-triton-multiarch` | x86_64 | pip install (torch, transformers, fastapi), CVCUDA 0.16.0 .deb install, Triton server entrypoint |
| cradio | `cradio/Dockerfile` | `nvcr.io/nvidia/deepstream:9.0-triton-multiarch` | x86_64 | TensorRT OSS build (v10.14), DeepStream TAO post-processors build, pip install (torch, transformers, polygraphy, tensorrt) |
| ds_app | `ds_app/Dockerfile` | `nvcr.io/nvidia/deepstream:9.0-triton-multiarch` | x86_64 | TensorRT OSS build (v10.14, archs 80/86/90/100/120), DeepStream TAO post-processors, pip install (torch, transformers, opencv) |
| ds_app (Jetson) | `ds_app/Dockerfile.tegra` | Multi-stage: `pytorch:25.08-py3` + `deepstream:9.0-triton-multiarch` | aarch64 (Jetson) | Multi-stage pytorch copy, TensorRT OSS build (v10.13, archs 110/120/121), DeepStream TAO post-processors |
| ds_app (DGX Spark) | `ds_app/Dockerfile.dgxspark` | Multi-stage: `pytorch:25.08-py3` + `deepstream:9.0-triton-sbsa-dgx-spark` | aarch64 (DGX SBSA) | Multi-stage pytorch copy, TensorRT OSS build (v10.13, archs 110/120/121), DeepStream TAO post-processors |
| vllm | `vllm/Dockerfile` | Multi-stage: `deepstream:9.0-triton-multiarch` + `vllm:26.02-py3` | x86_64 | Multi-stage DeepStream libs copy, pip install (qwen-vl-utils, aiohttp, omegaconf), custom shell entrypoint |
| nvclip | `nvclip/Dockerfile` | `nvcr.io/nvidia/deepstream:9.0-triton-multiarch` | x86_64 | pip install (torch, transformers, tensorrt, polygraphy, fastapi), user/group setup, custom shell entrypoint |
...
| tao | `tao/Dockerfile` | `nvcr.io/nvidia/deepstream:9.0-triton-multiarch` | x86_64 | TensorRT OSS build (v10.14, archs 80/86/90/100/120), DeepStream TAO post-processors, TAO symlink preparation script |
...
| qwen | `qwen/Dockerfile` | Multi-stage: `deepstream:9.0-triton-multiarch` + `tensorrt-llm/release:1.1.0` | x86_64 | Multi-stage DeepStream libs copy, pip install (qwen-vl-utils, aiohttp, omegaconf), TensorRT-LLM + DeepStream integration |

### Common Patterns

- **Base image**: Choose based on the primary inference stack — DeepStream/Triton pipelines use `nvcr.io/nvidia/deepstream:9.0-triton-multiarch` (x86_64) or its SBSA/DGX Spark variant (aarch64); vLLM pipelines use `nvcr.io/nvidia/vllm:26.02-py3` as the final stage; TensorRT-LLM pipelines use `nvcr.io/nvidia/tensorrt-llm/release:1.1.0` as the final stage. ARM64 targets (Jetson, DGX Spark) further add `nvcr.io/nvidia/pytorch` as a builder stage to supply pre-built ARM wheels.
- **TensorRT OSS**: DeepStream pipelines compile TensorRT OSS in-container for custom plugin support. GPU architecture list differs by platform — x86_64 uses `80;86;90;100;120`, ARM64 uses `110;120;121`.
