...
def parse_arguments():
    ...
    parser.add_argument('--export_onnx_only',
                        type=bool,
                        default=False,
                        help="Export into ONNX only")
...
def build_vila_engine(args):
    ...
    if isinstance(image, torch.Tensor):
        ...
        image = image[0].to(args.device, torch.float16)
        image = image.unsqueeze(0)
        # [1, 3, H, W]
    else:
        assert 0, f"unexpected image type: {type(image)}"
    ...
    del model
    export_visual_wrapper_onnx(wrapper, image, args.output_dir)
    del wrapper, vision_tower, image_processor
...
if __name__ == '__main__':
    """ Usage
        $ python build_visual_engine.py --model_path /workspace/checkpoints/baseline/vila1.5-40b/ --model_type vila \
        --max_batch_size 4 --vila_path ../ --output_dir /workspace/checkpoints/optimized/vila1.5-40b/fp16/1-gpu/visual_engines \
