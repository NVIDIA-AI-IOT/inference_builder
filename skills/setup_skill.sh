...
# Tool (builder CLI) version — repo root VERSION file
VERSION_FILE="${PROJECT_ROOT}/VERSION"
if [ ! -f "$VERSION_FILE" ]; then
    echo -e "${RED}[ERROR]${NC} VERSION file not found: $VERSION_FILE"
    exit 1
fi
TOOL_VERSION=$(cat "$VERSION_FILE" | tr -d '[:space:]')
...
read_skill_version() {
    local file="$1"
    awk '
        /^---$/ { if (++dashes > 2) exit; next }
        dashes == 1 && /^version:[[:space:]]*/ {
            sub(/^version:[[:space:]]*/, "")
            gsub(/^"/, "")
            gsub(/"$/, "")
            print
...
validate_sources() {
    ...
    if [ -z "$SKILL_VERSION" ]; then
        log_error "Could not parse skill version from SKILL.md frontmatter (expected version: \"…\")."
        exit 1
    fi
}
...
copy_base_files() {
    ...
    # Copy SKILL.md as-is (skill version lives in frontmatter; tool version in .skill_config)
    cp "$SKILL_MD" "$DEST_DIR/SKILL.md"
    log_info "Copied SKILL.md (skill version: $SKILL_VERSION, tool version: $TOOL_VERSION)"
...
INFERENCE_BUILDER_VERSION="$TOOL_VERSION"
...
EOF
    log_info "Created .skill_config (tool version: $TOOL_VERSION, project root: $PROJECT_ROOT)"
}
