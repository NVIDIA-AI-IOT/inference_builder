...
- `TYPE_CUSTOM_VIDEO_OUTPUT`: Output-only custom type for model-generated video frames. When used as a top-level output, it creates a `VideoOutputDataFlow` that accepts a `torch.Tensor` frame or list of frames in HWC `uint8` RGB layout `[H, W, 3]`, validates that all frames have matching dimensions, encodes them into an H.264 MP4 file at 30 fps with pyservicemaker nvenc, registers the MP4 with `AssetManager`, and returns the resulting `asset_id` string. The caller can use that asset ID with the asset management endpoints to retrieve the encoded video file.
